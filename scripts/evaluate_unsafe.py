#!/usr/bin/env python3
"""Evaluate PPO policy without safety filter (for comparison)."""
import argparse
import numpy as np
from rl_locomotion_cbf import create_quadruped_env, PPOPolicy


def main():
    """Main evaluation function."""
    parser = argparse.ArgumentParser(description='Evaluate unsafe PPO policy')
    ...
    # Evaluate
    print(f"\nEvaluating for {args.episodes} episodes WITHOUT safety filter")
    ...
    for episode in range(args.episodes):
        ...
        while not done:
            # Get action from policy (no safety filter)
    ...
    # Print statistics
    print("\n" + "="*60)
    print("EVALUATION RESULTS (WITHOUT SAFETY)")
    print("="*60)
